...
describe('shouldBlock', () => {
	...
	it('should not block if outside schedule', () => {
		...
		const settings = createSettings([{ domain: 'example.com', allowedPaths: [], blockedPaths: [] }])
		expect(shouldBlock('https://example.com', settings)).toBe(false)
	})

	it('should not block if domain is not in blockedSites', () => {
		const settings = createSettings([{ domain: 'facebook.com', allowedPaths: [], blockedPaths: [] }])
		expect(shouldBlock('https://google.com', settings)).toBe(false)
	})

	it('should match domain and subdomains', () => {
		const settings = createSettings([{ domain: 'example.com', allowedPaths: [], blockedPaths: [] }])
		// Default behavior (strict): block everything if allowedPaths is empty
	...
	describe('Strict Mode (allowOnlySubpaths = false/undefined)', () => {
		...
		it('should allow specific allowedPaths', () => {
			const settings = createSettings([
				{
					domain: 'example.com',
					allowedPaths: ['/messages', '/profile'],
					blockedPaths: []
				}
			])
		...
		it('should handle regex in allowedPaths', () => {
			const settings = createSettings([
				{
					domain: 'example.com',
					allowedPaths: ['/^\\/item\\/\\d+$/'], // Matches /item/123
					blockedPaths: []
				}
			])
	...
	describe('Permissive Mode (allowOnlySubpaths = true)', () => {
		// Logic: Block root explicitly. Allow others unless in blockedPaths.

		it('should block root path', () => {
			const settings = createSettings([
				{
					domain: 'example.com',
					allowOnlySubpaths: true,
					allowedPaths: [],
					blockedPaths: []
				}
			])
		...
		it('should block specific blockedPaths', () => {
			const settings = createSettings([
				{
					domain: 'example.com',
					allowOnlySubpaths: true,
					allowedPaths: [],
					blockedPaths: ['/shorts', '/reels']
				}
			])
		...
		it('should handle regex in blockedPaths', () => {
			const settings = createSettings([
				{
					domain: 'example.com',
					allowOnlySubpaths: true,
